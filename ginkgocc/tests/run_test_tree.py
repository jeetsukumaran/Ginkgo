...
class ParentArrayTreeRoundTripTest(unittest.TestCase):
    def setUp(self):
        self.prog_path = get_ginkgo_program_path("test_tree_from_parent_indexes")
        self.src_trees = [
    ...
    def testRoundTripParentArray(self):
        _LOG.info('Testing round-tripping of trees into GINKGO parent array and back')
        for t in self.src_trees:
            self.check_tree(t)
            
class GenealogyTreeTest(unittest.TestCase):
    def setUp(self):
        self.prog_path = get_ginkgo_program_path("test_tree_from_genealogies")
